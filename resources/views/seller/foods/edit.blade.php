...
<body>
    ...
    <div class="result_wrap">
        <form id="art_form" action="{{url('seller/foods/'.$foods->foods_id)}}" method="post" enctype="multipart/form-data">
            <table class="add_tab">
                <tbody>
                    {{csrf_field()}}
                    ...
                    <tr>
                        <th>菜品所属套餐：</th>
                            <td>
                                ...
                                @foreach($taocan as $v)  
                                @if($foods->taocan_id == $v->taocan_id)  
                                    <option value="{{ $v->taocan_id }}" selected="selected" >　｜－－{{ $v->taocan_name }}</option>
                                @else
